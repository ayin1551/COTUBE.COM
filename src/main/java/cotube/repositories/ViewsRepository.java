...
public interface ViewsRepository extends CrudRepository<Views, Integer> {
    ...
    @Query(value = "select v.comic_id from Views v where v.comic_id in (select c.comic_id from comic c where c.comic_type = 0) Group by v.comic_id oRDER BY count(v.comic_id) DESC LIMIT 20", nativeQuery = true)
